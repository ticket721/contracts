...
const EventRegistry = async () => {
    return new Promise((ok, ko) => {
        ...
        exec(`./node_modules/.bin/zos create EventRegistry --init initialize --args ${admin_artifact.networks[config.network_id].address},${emr_artifact.networks[config.network_id].address} --network ${process.env.T721_NETWORK}`,
            (err, stdout, stderr) => {
                if (err) {
